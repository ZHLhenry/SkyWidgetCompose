 ...
 */
internal class SkyRefreshNestedScrollConnection(
    private val state: SkyRefreshState,
    ...
    override fun onPreScroll(available: Offset, source: NestedScrollSource): Offset {
        ...
        return when {
            ...
            availableAxis > 0 && (state.indicatorOffset < 0f || (state.isContentOffsetDecoupled && state.decoupledContentOffset < 0f)) -> {
                var consumed = 0f
                // 1. 收起 Footer（Footer 全程 1:1 线性跟手，与拉出方向手感对称）
    ...
    override fun onPostScroll(
        ...
        return when {
            ...
            availableAxis > 0 && state.enableRefresh && state.headerBound > 0f && state.refreshFlag != SkyRefreshFlag.FINISHING && state.loadMoreFlag != SkyRefreshFlag.REFRESHING -> {
                ...
                state.indicatorOffset += delta
                if (state.refreshFlag == SkyRefreshFlag.IDLE && state.indicatorOffset > 0) {
                    state.refreshFlag = SkyRefreshFlag.PULLING
                    // 状态互斥：清理 Footer 侧可能残留的 PULLING 标记
                    if (state.loadMoreFlag == SkyRefreshFlag.PULLING) {
            ...
            // 互斥约束：下拉刷新执行期间（REFRESHING），禁止拉出 Footer
            availableAxis < 0 && state.enableLoadMore && state.footerBound > 0f && state.loadMoreFlag != SkyRefreshFlag.FINISHING && state.refreshFlag != SkyRefreshFlag.REFRESHING -> {
                val limit = state.footerBound
                // 物理特性：Footer 侧不做阻尼衰减，全程 1:1 线性跟手。
                // 触发阈值是 Footer 完全可见（-footerBound），若叠加 0.5 阻尼，
                // 用户需滑动 2 倍 Footer 高度才能触发加载，拖拽成本过高。
                val stickiness = 1f
                ...
                if (!state.noMoreData) {
                    if (state.loadMoreFlag == SkyRefreshFlag.IDLE && state.indicatorOffset < 0) {
                        state.loadMoreFlag = SkyRefreshFlag.PULLING
                        // 状态互斥：清理 Header 侧可能残留的 PULLING 标记
                        if (state.refreshFlag == SkyRefreshFlag.PULLING) {
                            state.refreshFlag = SkyRefreshFlag.IDLE
                        }
                    }
                    if (state.indicatorOffset <= -state.footerBound && state.loadMoreFlag != SkyRefreshFlag.REFRESHING) {
                        state.loadMoreFlag = SkyRefreshFlag.REFRESHING
                        onLoadMore()
                    }
                }
    ...
    override suspend fun onPreFling(available: Velocity): Velocity {
        ...
        if (state.refreshFlag == SkyRefreshFlag.REFRESHING) {
            ...
            // 预加载已在 onPostScroll 中触发过 onLoadMore（此时 offset == -footerBound），
            // 必须在此拦截返回，防止穿透到下方 footerOver 分支导致回调被重复触发
            return Velocity.Zero
        }
        return when {
            ...
            footerOver && !state.noMoreData && state.loadMoreFlag != SkyRefreshFlag.FINISHING -> {
                state.loadMoreFlag = SkyRefreshFlag.REFRESHING
                scope.launch { state.animateOffsetTo(-state.footerBound) }
                onLoadMore()
                available.mainAxis().toVelocity()
    ...
    override suspend fun onPostFling(consumed: Velocity, available: Velocity): Velocity {
        ...
        // 手势在一侧 PULLING 期间可能反向拉到另一侧，松手时两侧标记都需清理，保证互斥
        ...
        // 处理底部的回弹逻辑
        if (state.loadMoreFlag != SkyRefreshFlag.REFRESHING && state.indicatorOffset < 0f) {
            if (state.noMoreData) {
                // 仅在惯性滚动触底（本次 Fling 带有真实速度）时按过半规则决策停靠/收起；
                // 手指松开的场景已在 onPreFling 中完成决策，避免二次决策互相干扰
