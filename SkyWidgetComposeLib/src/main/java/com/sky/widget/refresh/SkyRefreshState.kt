...
class SkyRefreshState(
     ...
     * 该状态 **仅应通过 [finish] 来变更**，不与下拉刷新逻辑耦合。
     */
    ...
    /** 顶部下拉刷新的有限状态机 (FSM) 标识 */
    private var _refreshFlag by mutableStateOf(SkyRefreshFlag.IDLE)
    var refreshFlag: SkyRefreshFlag
        get() = _refreshFlag
        internal set(value) {
            _refreshFlag = value
            ...
            // 从而使后续上拉加载重新生效。
            if (value == SkyRefreshFlag.REFRESHING) {
                noMoreData = false
            }
        }

    /** 底部上拉加载的有限状态机 (FSM) 标识 */
    var loadMoreFlag by mutableStateOf(SkyRefreshFlag.IDLE)
        internal set

    /**
     * 业务回调触发器：由容器层注入，与手势链路共用同一回调。
     * 供 [autoRefresh] / [autoLoadMore] 程序化触发时向业务层派发事件。
     */
    internal var refreshAction: (() -> Unit)? = null
    internal var loadMoreAction: (() -> Unit)? = null
     ...
     * 避免调用方在同一接口回调中先后调用 loadMore 与 refresh 的结束方法时，
     * 后者的默认参数无意间覆盖前者已设置的"无更多数据"标记。
     *
     * 如需在下拉刷新完成后重置无更多数据状态，请在业务层显式调用
     * `finish(noMoreData = false)`。
     */
    private suspend fun finishRefresh() {
        if (refreshFlag == SkyRefreshFlag.IDLE) return
    ...
    private suspend fun finishLoadMore(noMoreData: Boolean = false) {
        ...
        if (loadMoreFlag == SkyRefreshFlag.IDLE) return
        loadMoreFlag = SkyRefreshFlag.FINISHING
        ...
        if (noMoreData && footerBound > 0f) {
            // 数据穷尽：平滑收起 Footer，不在视口底部常驻悬浮。
            // “没有更多数据”提示仅当用户滑动到列表末尾时，
            // 可通过上拉手势以 1:1 跟手方式重新拉出查看。
        ...
        loadMoreFlag = SkyRefreshFlag.IDLE
    }
    ...
    suspend fun autoLoadMore() {
        // 互斥约束：下拉刷新执行期间禁止触发上拉加载
        if (noMoreData || loadMoreFlag != SkyRefreshFlag.IDLE || refreshFlag == SkyRefreshFlag.REFRESHING || footerBound <= 0f) return
        loadMoreFlag = SkyRefreshFlag.REFRESHING
    ...
    suspend fun autoRefresh() {
        // 互斥约束：上拉加载执行期间禁止触发下拉刷新
        if (refreshFlag != SkyRefreshFlag.IDLE || loadMoreFlag == SkyRefreshFlag.REFRESHING || headerBound <= 0f) return
        refreshFlag = SkyRefreshFlag.REFRESHING
...
/**
 * 构建并持久化一个 [SkyRefreshState] 实例，与所在组件的生命周期绑定。
 *
