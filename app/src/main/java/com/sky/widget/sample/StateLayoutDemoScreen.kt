...
fun StateLayoutDemoScreen(onBack: () -> Unit) {
    ...
    var pageState by remember { mutableStateOf<SkyPageState>(SkyPageState.Loading) }
        ...
        Column(
            ...
            Row(
                ...
                StateButton(
                    ...
                    onClick = { pageState = SkyPageState.Loading }
                )
                StateButton(
                    text = "Success",
                    modifier = Modifier.weight(1f),
                    onClick = { pageState = SkyPageState.Success }
                )
                StateButton(
                    text = "Empty",
                    modifier = Modifier.weight(1f),
                    onClick = { pageState = SkyPageState.Empty("没有搜索到相关结果") }
                )
                StateButton(
                    text = "Error",
                    modifier = Modifier.weight(1f),
                    onClick = { pageState = SkyPageState.Error("服务器开小差了") }
                )
            }
        }
    ...
    // 首次进入自动加载
    if (pageState == SkyPageState.Loading && items.isEmpty()) {
        // 已由初始状态触发，无需额外操作
    }
}
